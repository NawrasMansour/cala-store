...
const Index = ({items = [] , ...props}) => {
  ...
  const handleCheckboxFilter = (e) => {
    const splitedID = e.target.id.split("-");
    switch (splitedID[1]) {
      case 'category':
        filters[0].options = filters[0].options.map( (option , index) => {
          if(index == splitedID[2])
            option.checked = !option.checked
          return option;
        })
        break;
      case 'cluster':
        filters[1].options = filters[1].options.map( (option , index) => {
          if(index == splitedID[2])
            option.checked = !option.checked
          return option;
        })
        break;
      case 'group':
        filters[2].options = filters[2].options.map( (option , index) => {
          if(index == splitedID[2])
            option.checked = !option.checked
          return option;
        })
        break;
      case 'color':
        filters[3].options = filters[3].options.map( (option , index) => {
          if(index == splitedID[2])
            option.checked = !option.checked
          return option;
        })
    ...
    setFilter(filters);
    console.log(filters);
    return Inertia.get(route('products.shop.index'));
  }
          ...
          <section aria-labelledby="products-heading" className="pt-6 pb-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-x-8 gap-y-10">
              ...
              <div className="lg:col-span-3">
                {items.data.length > 0 &&
                  <ItemsGrid title="" data={items.data} pagination={items} />
                }

                {
                  items.data.length == 0 && <EmptySearchData />
                }
