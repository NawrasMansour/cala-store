...
const Edit = (props) => {
  ...
  const { data, setData, post, processing, errors } = useForm({
        en_name: props.cluster.en_name || '',
        ar_name: props.cluster.ar_name || '',
        en_description:props.cluster.en_description || '',
        ar_description:props.cluster.ar_description || '',
        picture: '' as any,
        sub_category_id : props.cluster.sub_category_id,
        _method: 'PUT',
    });
  ...
  const submit = (e)=>{
    ...
    post(route(`${props.routeBaseName}.update`, props.cluster.id), {
            preserveState: (page) =>
            ...
            <select className="form-select form-select-lg mb-3 appearance-none block w-full px-4 py-2 text-xl font-normal text-gray-700
              ...
              >
                {props.subCategories.map((subCategory) => {
                  return subCategory.id == data.sub_category_id ?
                  ( <option key={subCategory.id} value={subCategory.id} selected>{subCategory.en_name}</option>)
                  : ( <option key={subCategory.id} value={subCategory.id}>{subCategory.en_name}</option>)
                })}
            </select>
