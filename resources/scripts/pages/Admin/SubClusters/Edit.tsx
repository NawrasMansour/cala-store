...
const Edit = (props) => {
  ...
  const { data, setData, post, processing, errors } = useForm({
        en_name: props.subCluster.en_name || '',
        ar_name: props.subCluster.ar_name || '',
        en_description:props.subCluster.en_description || '',
        ar_description:props.subCluster.ar_description || '',
        picture: '' as any,
        cluster_id : props.subCluster.cluster_id,
        _method: 'PUT',
    });
  ...
  const submit = (e)=>{
    ...
    post(route(`${props.routeBaseName}.update`, props.subCluster.id), {
            preserveState: (page) =>
        ...
        <div className='flex flex-row'>
          <div className='p-2 w-full'>
            ...
            <select className="form-select form-select-lg mb-3 appearance-none block w-full px-4 py-2 text-xl font-normal text-gray-700
              ...
              onChange={(e) => {
                  setData('cluster_id', e.target.value);
              }}
              >
                {props.clusters.map((cluster) => {
                  return cluster.id == data.cluster_id ?
                  ( <option key={cluster.id} value={cluster.id} selected>{cluster.en_name}</option>)
                  : ( <option key={cluster.id} value={cluster.id}>{cluster.en_name}</option>)
                })}
            </select>
            {errors.cluster_id && (
              <div className="mb-3 text-normal text-red-500 ">
