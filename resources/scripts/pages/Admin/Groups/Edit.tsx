...
const Edit = (props) => {
  ...
  const { data, setData, post, processing, errors } = useForm({
        en_name: props.group.en_name || '',
        ar_name: props.group.ar_name || '',
        en_description:props.group.en_description || '',
        ar_description:props.group.ar_description || '',
        picture: '' as any,
        sub_cluster_id : props.group.sub_cluster_id,
        _method: 'PUT',
    });
  ...
  const submit = (e)=>{
    ...
    post(route(`${props.routeBaseName}.update`, props.group.id), {
            preserveState: (page) =>
        ...
        <div className='flex flex-row'>
          <div className='p-2 w-full'>
            ...
            <select className="form-select form-select-lg mb-3 appearance-none block w-full px-4 py-2 text-xl font-normal text-gray-700
              ...
              onChange={(e) => {
                  setData('sub_cluster_id', e.target.value);
              }}
              >
                {props.subClusters.map((subCluster) => {
                  return subCluster.id == data.sub_cluster_id ?
                  ( <option key={subCluster.id} value={subCluster.id} selected>{subCluster.en_name}</option>)
                  : ( <option key={subCluster.id} value={subCluster.id}>{subCluster.en_name}</option>)
                })}
            </select>
            {errors.sub_cluster_id && (
              <div className="mb-3 text-normal text-red-500 ">
                {errors.sub_cluster_id}
              </div>
            )}
