...
const Edit = (props) => {
  ...
  const { data, setData, post, processing, errors } = useForm({
        en_name: props.subCategory.en_name || '',
        ar_name: props.subCategory.ar_name || '',
        en_description:props.subCategory.en_description || '',
        ar_description:props.subCategory.ar_description || '',
        picture: '' as any,
  ...
  const submit = (e)=>{
    ...
    post(route(`${props.routeBaseName}.update`, props.subCategory.id), {
            preserveState: (page) =>
            ...
            <select className="form-select form-select-lg mb-3 appearance-none block w-full px-4 py-2 text-xl font-normal text-gray-700
              ...
              >
                {props.categories.map((category) => {
                  ...
                  ( <option key={category.id} value={category.id} selected>{category.en_name}</option>)
