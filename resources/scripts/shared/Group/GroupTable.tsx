...
const GroupTable = ({groups ,pagination ,routeBaseName}) => {
  ...
  const groupsData = groups?.map((group) => ({
        ...
        details: (
            <button
                  ...
                  "
                  onClick={() =>
                    Inertia.get(route(`${routeBaseName}.show`, group.id))
                }
                >
                  More
                </button>
                    ...
                    <table className="min-w-full text-center ">
                        ...
                        <tbody>
                            {groupsData.map( (groupData)=>(
                                <tr key={groupData.id} className="bg-white border-b hover:bg-gray-100">
                                    <td className="px-6 py-2 whitespace-nowrap text-sm font-light text-gray-900">{groupData.id}</td>
