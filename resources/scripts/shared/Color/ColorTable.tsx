...
const ColorTable = ({colors ,pagination ,routeBaseName}) => {
  ...
  const colorsData = colors?.map((color) => ({
        ...
        details: (
            <button
                  ...
                  "
                  onClick={() =>
                    Inertia.get(route(`${routeBaseName}.show`, color.id))
                }
                >
                  More
                </button>
                    ...
                    <table className="min-w-full text-center ">
                        ...
                        <tbody>
                            {colorsData.map( (colorData)=>(
                                <tr key={colorData.id} className="bg-white border-b hover:bg-gray-100">
                                    <td className="px-6 py-2 whitespace-nowrap text-sm font-light text-gray-900">{colorData.id}</td>
                                    <td className="text-sm text-gray-900 font-medium px-6 py-2 whitespace-nowrap">
                                        {colorData.en_name}
                                    </td>
                                    <td className="text-sm text-gray-900 font-light px-6 py-2 whitespace-nowrap">
                                        {colorData.created_at}
                                    </td>
